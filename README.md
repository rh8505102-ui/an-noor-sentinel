...
import time
# --- নতুন ৩টি লাইন (Render-এ বটকে জাগিয়ে রাখার জন্য) ---
...
def ask_gemini(user_text):
    ...
    system_instruction = "You are an Advanced Islamic Scholar. Provide knowledge from basic to advanced in Bangla. Use Quran & Sahih Hadith."
    payload = {"contents": [{"parts": [{"text": f"{system_instruction}\n\nUser Question: {user_text}"}]}]}
    try:
        response = requests.post(url, json=payload, timeout=30)
        return response.json()['candidates'][0]['content']['parts'][0]['text']
    except:
        return "⚠️ সার্ভার ব্যস্ত। ইস্তিগফার পড়ুন এবং কিছুক্ষণ পর চেষ্টা করুন।"
